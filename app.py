...
# Estado de la conversación para cada usuario (almacenamiento básico en memoria)
...
def cotejar_sintomas(sintomas_usuario, hoja):
    """Coteja los síntomas del usuario con Google Sheets."""
    try:
        registros = hoja.get_all_records()
        coincidencias = [registro for registro in registros if any(sintoma in registro.values() for sintoma in sintomas_usuario)]
        if coincidencias:
            # Encontrar el posible diagnóstico (columna "D")
            diagnosticos = list(set([c["D"] for c in coincidencias if "D" in c]))
            return diagnosticos
        return []
    except Exception as e:
...
def asistente():
    ...
    try:
        ...
        session_id = data.get("session_id", "default")  # Identificador de sesión o conversación
        logging.debug(f"Mensaje recibido: {mensaje_usuario}")

        # Inicializar estado de la conversación si es nuevo
        if session_id not in estado_conversaciones:
            estado_conversaciones[session_id] = {"sintomas": [], "fase": "saludo"}

        estado = estado_conversaciones[session_id]
        ...
        if estado["fase"] == "saludo":
            estado["fase"] = "preguntar_sintomas"
            return jsonify({"respuesta": random.choice(saludos)})

        # Detectar agradecimientos o fin de conversación
        if mensaje_usuario in ["gracias", "muchas gracias"] or mensaje_usuario in respuestas_fin_usuario:
            del estado_conversaciones[session_id]  # Terminar la conversación
            return jsonify({"respuesta": random.choice(respuestas_finales)})

        # Registrar los síntomas del usuario
        estado["sintomas"].append(mensaje_usuario)

        # Cotejar síntomas con Google Sheets
        diagnosticos = cotejar_sintomas(estado["sintomas"], hoja)

        if diagnosticos:
            # Usar OpenAI para enriquecer la respuesta
            sintomas_mencionados = ", ".join(estado["sintomas"])
            ...
            del estado_conversaciones[session_id]  # Terminar conversación tras diagnóstico
            return jsonify({"respuesta": respuesta})

        # Registrar nuevos síntomas en Google Sheets si no hay coincidencias
        hoja.append_row(estado["sintomas"] + ["", "", "", ""])
